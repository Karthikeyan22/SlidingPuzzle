...
function start() {
    ...
    function play() {
        ...
        for (var i = 0; i < 6; i++) {
            for (var j = 0; j <6; j++) {
                ...
                cell.id = 'cell-' + i + '-' + j;
                cell.style.left = (j * 70 + 1 * j + 1) + 'px';
                cell.style.top = (i * 70 + 1 * i + 1) + 'px';
    ...
    function getAdjacentCells(cell) {
        ...
        var adjacent = [];
        if (row < 5) { adjacent.push(getCell(row + 1, col)); }
        if (row > 0) { adjacent.push(getCell(row - 1, col)); }
        if (col < 5) { adjacent.push(getCell(row, col + 1)); }
    ...
    function checkOrder() {
        ...
        for (var i = 0; i < 6; i++) {
            for (var j = 0; j < 6; j++) {
                if (n <36 && getCell(i, j).innerHTML != n.toString()) {
